...
kable(value_bydescription, "markdown", format.args = list(big.mark = ",", 
  scientific = FALSE), align = "r")
...
rm(value_bydescriptionyear, value_bydescriptionyear01, value_bydescriptionyear02,
   value_bydescriptionyear03)
```


### C. Average unit values ###
10)    Average value of import by taxa (“value”/”quantity” for those with “NO” [number] in the “unit” column) by “taxa”
```{r eval=TRUE, echo=TRUE, warning=FALSE, include=TRUE}
avgimport_bytaxa_no<- data %>% 
  ...
  group_by(taxa) %>%
  filter(!is.na(import))  %>%
  summarise(import_mean=mean(as.numeric(import)), 
            import_median = median(as.numeric(import))) %>%
  arrange(desc(import_mean))

kable(avgimport_bytaxa_no, "markdown", format.args = list(big.mark = ",", 
  scientific = FALSE), align = "r")

ggplot(avgimport_bytaxa_no, aes(x = reorder(taxa, -import), y=value)) + 
       geom_bar(stat = "identity", width=0.4, position = position_dodge(width=0.5), 
           fill="blue") +
       scale_y_continuous(labels = scales::dollar_format(prefix="$")) +
       geom_text(aes(label = scales::comma(value)), vjust=-1, size=2) +
       labs(y = "Import",  x="Taxa") 
...
kable(avgimport_bytaxa_kg, "markdown", format.args = list(big.mark = ",", 
  scientific = FALSE), align = "r")

ggplot(avgimport_bytaxa_kg, aes(x = reorder(taxa, -import), y=value)) + 
       geom_bar(stat = "identity", width=0.4, position = position_dodge(width=0.5), 
           fill="blue") +
       scale_y_continuous(labels = scales::dollar_format(prefix="$")) +
       geom_text(aes(label = scales::comma(value)), vjust=-1, size=2) +
